...
from data import load_caps, load_docs, load_file, load_img_doc,ClueWebDataset
...
def train(train_reader, valid_reader, model, device, writer: SummaryWriter):
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, batch in enumerate(train_reader):
            ...
            pos_embeddings = torch.index_select(candidate_embeddings, 0, torch.LongTensor(batch['pos_idx']).cuda())  #batch['pos_idx']:(list) batch_size  -->pos_embeddings batch_size *768
            neg_embeddings = torch.index_select(candidate_embeddings, 0, torch.LongTensor(batch['neg_idx']).cuda())
...
if __name__ == '__main__':
    ...
    train_data = ClueWebDataset(args, image_processor, tokenizer, train_data, docs, img_doc,captions=captions, shuffle=True,img_special_len=img_patch_token_size)
    ...
    args_dev.neg_num = 1
    valid_data = ClueWebDataset(args_dev,image_processor, tokenizer, valid_data, docs, img_doc,captions=captions, shuffle=False,img_special_len=img_patch_token_size)
